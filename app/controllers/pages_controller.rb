class PagesController < ApplicationController
    ...
    def search
        ...
        if !params[:search].blank? && params[:search].length > 1
            ...
            @article_articles = Article.where(:published => true).where('lower(body) LIKE lower(?) OR lower(title) LIKE lower(?) OR lower(by) LIKE lower(?) OR lower(illustrator) LIKE lower(?)', "%#{search}%", "%#{search}%", "%#{search}%", "%#{search}%").order("published_at DESC")
